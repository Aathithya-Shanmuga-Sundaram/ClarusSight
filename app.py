...
with st.sidebar:
    ...
    if st.button("📊 Export Report", use_container_width=True):
        ...
        threats_list = st.session_state.threats
        
        # Create CSV content
        csv_content = "Threat_ID,Timestamp,Type,Severity,Actor,Target_Industry,CVSS_Score,Affected_Systems,Status\n"
        for threat in threats_list:
            csv_content += f"{threat['id']},{threat['timestamp']},{threat['type']},{threat['severity']},"
            csv_content += f"{threat['threat_actor']},{threat['target_industry']},{threat['cvss_score']:.2f},"
            csv_content += f"{threat['affected_systems']},{threat['mitigation_status']}\n"
        
        st.download_button(
            label="📥 Download Threat Data (CSV)",
            data=csv_content,
            file_name=f"CTI_Threat_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
    ...
    if st.button("📋 Export IOC List", use_container_width=True):
        # Generate downloadable IOC list
        ioc_list = st.session_state.iocs
        
        ioc_content = "IOC_Type,Value,Threat_Level,First_Seen,Last_Seen,Associated_Threats\n"
        for ioc in ioc_list:
            ioc_content += f"{ioc['type']},{ioc['value']},{ioc['threat_level']},"
            ioc_content += f"{ioc['first_seen']},{ioc['last_seen']},{ioc['associated_threats']}\n"
        
        st.download_button(
            label="📥 Download IOC Database (CSV)",
            data=ioc_content,
            file_name=f"CTI_IOC_List_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True
        )
...
if len(threats_df) == 0:
    # Show prominent empty state message
    st.warning("⚠️ **No threats found matching your filters!**")
    st.markdown("""
    <div style='text-align: center; padding: 40px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
                border-radius: 15px; margin: 20px 0;'>
        <h2 style='color: #00d9ff; font-size: 2.5em; margin: 0;'>🔍</h2>
        <h3 style='color: white; margin: 20px 0;'>No Results</h3>
        <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
            Your current filters exclude all threats from the dataset.
        </p>
        <p style='color: #00d9ff; margin: 20px 0;'>
            💡 Try: Broadening your filters, changing the time range, or clearing all filters
        </p>
    </div>
    """, unsafe_allow_html=True)
elif len(threats_df) < len(st.session_state.threats):
...
with tabs[0]:
    ...
    if len(threats_df) == 0:
        st.markdown("""
        <div style='text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
                    border-radius: 15px; margin: 40px 0;'>
            <h2 style='color: #00d9ff; font-size: 3em; margin: 0;'>🎉</h2>
            <h3 style='color: white; margin: 20px 0;'>No Threats Found</h3>
            <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
                No threats match your current filter criteria.
            </p>
            <p style='color: #00d9ff; margin: 20px 0;'>
                Try adjusting your filters in the sidebar or refresh the data.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            if threat_filter:
                st.info(f"**Threat Types:** {', '.join(threat_filter)}")
            else:
                st.info("**Threat Types:** All")
        with col3:
            if severity_filter:
                st.info(f"**Severity:** {', '.join(severity_filter)}")
            else:
                st.info("**Severity:** All")
    else:
        # Show normal dashboard when threats exist
        # Key Metrics
...
with tabs[1]:
    ...
    # Check if there are threats to predict from
    if len(threats_df) == 0:
        st.markdown("""
        <div style='text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
                    border-radius: 15px; margin: 40px 0;'>
            <h2 style='color: #00d9ff; font-size: 3em; margin: 0;'>🔮</h2>
            <h3 style='color: white; margin: 20px 0;'>No Data to Predict From</h3>
            <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
                Predictive models require historical threat data.
            </p>
            <p style='color: #00d9ff; margin: 20px 0;'>
                Adjust your filters to see 24-hour threat forecasts.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        # Recalculate predictions from filtered threats
        if len(threats_df) > 0:
...
with tabs[2]:
    ...
    # Check if there are threats to analyze
    if len(threats_df) == 0:
        st.markdown("""
        <div style='text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
                    border-radius: 15px; margin: 40px 0;'>
            <h2 style='color: #00d9ff; font-size: 3em; margin: 0;'>🕵️</h2>
            <h3 style='color: white; margin: 20px 0;'>No Data to Analyze</h3>
            <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
                Anomaly detection requires threat data to analyze patterns.
            </p>
            <p style='color: #00d9ff; margin: 20px 0;'>
                Adjust your filters to see behavioral anomalies.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        # Detect anomalies from filtered threats
        if len(threats_df) > 0:
        ...
        if len(anomalies_df) > 0:
            ...
            with col1:
                ...
                # Normal threats (background) - use filtered data
                normal_threats = threats_df
                fig.add_trace(go.Scatter(
...
with tabs[3]:
    ...
    # Build attack graph using filtered data
...
with tabs[4]:
    ...
    if len(threats_df) > 0:
        ...
        # Filter IOCs: only show IOCs with reasonable association to filtered threats
        # Since IOCs don't have direct threat_id links in our data model,
        # we'll filter based on the number of threats
        # IOCs associated with more threats are more likely to be relevant
        all_iocs_df = pd.DataFrame(st.session_state.iocs)
        
        # Calculate what percentage of threats are in our filter
        filter_ratio = len(threats_df) / len(st.session_state.threats)
        
        # Sample IOCs proportionally (this simulates filtering by association)
        num_iocs_to_show = int(len(all_iocs_df) * filter_ratio)
        if num_iocs_to_show < len(all_iocs_df):
            # Show IOCs with highest associated_threats first (most relevant)
            iocs_df = all_iocs_df.nlargest(num_iocs_to_show, 'associated_threats')
        else:
            iocs_df = all_iocs_df
    else:
    ...
    if len(threats_df) == 0:
        ...
        <div style='text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
                    border-radius: 15px; margin: 40px 0;'>
            <h2 style='color: #00d9ff; font-size: 3em; margin: 0;'>📋</h2>
            <h3 style='color: white; margin: 20px 0;'>No IOCs to Display</h3>
            <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
                IOCs are shown based on filtered threats.
            </p>
            <p style='color: #00d9ff; margin: 20px 0;'>
                Adjust your filters to see relevant indicators of compromise.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        if len(iocs_df) < len(st.session_state.iocs):
            st.info(f"📊 Showing {len(iocs_df)} IOCs associated with {len(threats_df)} filtered threats (out of {len(st.session_state.iocs)} total IOCs)")
...
with tabs[5]:
    ...
    if len(threats_df) == 0:
        ...
        <div style='text-align: center; padding: 60px 20px; background: linear-gradient(135deg, #1e2749 0%, #2d3561 100%); 
            ...
            <h3 style='color: white; margin: 20px 0;'>No Data to Analyze</h3>
            <p style='color: #a4b0be; font-size: 1.1em; margin: 10px 0;'>
                AI insights require threat data to analyze.
            </p>
            <p style='color: #00d9ff; margin: 20px 0;'>
                Adjust your filters to see AI-powered intelligence analysis.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        avg_cvss = threats_df['cvss_score'].mean() if len(threats_df) > 0 else 0.0
    
        # Safely get top actor and type
        actor_counts = threats_df['threat_actor'].value_counts()
        top_actor = actor_counts.index[0] if len(actor_counts) > 0 else "N/A"
    
        type_counts = threats_df['type'].value_counts()
        top_type = type_counts.index[0] if len(type_counts) > 0 else "N/A"
    
        # Safely get top industry
        top_industry = industry_threats.iloc[0]['Industry'] if len(industry_threats) > 0 else "N/A"
        ...
        <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;'>
            ...
            <div>
                ...
                <ul style='color: white; line-height: 1.8;'>
                    ...
                    <li>Review security controls for <strong>{top_industry}</strong> sector</li>
